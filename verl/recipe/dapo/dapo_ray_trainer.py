...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    # TODO: validate with custom val function
    def _custom_validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            # SeedBench format
            val_scores = result["val_score"]
            scores = [item["score"] for item in val_scores]
            sample_scores.extend(scores)
            
            reward_extra_infos_dict["reward"].extend(scores)
            print(f"len reward_extra_infos_dict['reward']: {len(reward_extra_infos_dict['reward'])}")
            
            # Extract question and question_type info
            questions = [item["question"] for item in val_scores]
            question_types = [item["question_type"] for item in val_scores]
            reward_extra_infos_dict["question"].extend(questions)
            reward_extra_infos_dict["question_type"].extend(question_types)
            print(f"len reward_extra_infos_dict['question']: {len(reward_extra_infos_dict['question'])}")
            print(f"len reward_extra_infos_dict['question_type']: {len(reward_extra_infos_dict['question_type'])}")
            
            # Handle additional extra info from SeedBench
            if "val_extra_info" in result:
                for key, lst in result["val_extra_info"].items():
                ...
                with marked_timer("step", timing_raw):
                    ...
                    if rollout_data_dir:
                        with marked_timer("dump_rollout_generations", timing_raw, color="green"):
                            ...
                            # 直接从 batch.non_tensor_batch 中提取 reward_extra_infos_dict
                            reward_extra_infos_batch = {}
                            for key, val in batch.non_tensor_batch.items():
                                # if key not in ["uid", "data_source", "seq_final_reward", "seq_reward"]:
                                #     reward_extra_infos_batch[key] = val.tolist() if isinstance(val, np.ndarray) else val

                                # outcome_acc: accuracy of outcome of ablation experiment[20250808-2120]
                                if key in ['score', 'accuracy_reward', 'format_reward', 'total_answer_reward', 'hard_answer_match_reward', 'soft_answer_match_reward', 'checklist_reward', 'outcome_acc']:
                                    reward_extra_infos_batch[key] = val.tolist() if isinstance(val, np.ndarray) else val
                            self._dump_generations(
                                inputs=inputs,
                                outputs=outputs,
                                scores=scores,
                                reward_extra_infos_dict=reward_extra_infos_batch,
