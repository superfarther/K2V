...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    # 添加 reward_extra_info 到 metrics
    for key in batch.non_tensor_batch.keys():
        if key in ['format_reward', 'accuracy_reward', 'checklist_reward', 'total_answer_reward', 'hard_answer_match_reward', 'soft_answer_match_reward']:
            values = batch.non_tensor_batch[key]
            if len(values) > 0:
                metrics[f"rewards_info/{key}/mean"] = np.mean(values)
                metrics[f"rewards_info/{key}/max"] = np.max(values)
                metrics[f"rewards_info/{key}/min"] = np.min(values)
